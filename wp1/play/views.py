...
def listen(request):
	...
	if request.META.get('CONTENT_TYPE') == 'application/json':
		if request.method == 'POST':
			keys_d = json.loads(request.body.decode('utf8'))
			keys = keys_d['keys']
	...
	def callback(ch, method, properties, body):
	    ...
	    channel.stop_consuming("listen_tag")
	  
	channel.basic_consume(callback,
	                      queue=queue_name,
	                      no_ack=True,
	                      consumer_tag = "listen_tag")

	channel.start_consuming()
	data = {"msg" : str(msg.decode("utf-8") )}
	return HttpResponse(json.dumps(data).encode('utf8'),content_type="application/json")
...
def speak(request):
	...
	if request.META.get('CONTENT_TYPE') == 'application/json':
		if request.method == 'POST':
			keymsg = json.loads(request.body.decode('utf8'))
			key = keymsg['key']
			msg = keymsg['msg']
...
def index2(request):
	...
	if request.META.get('CONTENT_TYPE') == 'application/json':
		if request.method == 'POST':
			...
			if 'SESSION' in request.COOKIES and move != None and move!="null":
				#print(move)
				#return HttpResponse("first"+str(move))
				session = request.COOKIES['SESSION']
				with open(os.path.dirname(os.path.realpath(__file__))[:-4]+'sessions.json','r') as jfile:
					old_ses = json.load(jfile)
				try:
					...
					if user['grid'][move] == ' ':
						...
						if 'winner' not in griddict:
							...
							with open(os.path.dirname(os.path.realpath(__file__))[:-4]+'accounts.json','w') as ofile:
							...
							for i in range(len(gamelists['gamelists'])):
								if username in gamelists['gamelists'][i]:
									...
									#return HttpResponse(str(gid)+"here")
									check_prev = True
									if gid >= 0:
										if gamelists['gamelists'][i][username][gid]['winner'] != 'None':
											check_prev = False
									if gid ==  -1 or check_prev:
										#return HttpResponse(str(gid)+"emm")
										gid += 1
										date = datetime.datetime.now().strftime("%y-%m-%d")
									
										#return HttpResponse(str(gamelists)+"wat")
										gamelists['gamelists'][i][username] += [ {'grid': griddict['grid'], 'id': gid+1, 'start_date':date, 'winner':griddict['winner']}]
										#return HttpResponse(str(gamelists))
									else:
										
										gamelists['gamelists'][i][username][gid+1]['grid'] = griddict['grid']
										gamelists['gamelists'][i][username][gid+1]['winner'] = griddict['winner']
							
									gid += 1 
									date = datetime.datetime.now().strftime("%y-%m-%d")
									gamelists['gamelists'][i][username] += [{'id':gid+1, 'grid':[' ',' ',' ',' ',' ',' ',' ', ' ',' '], 'start_date':date, 'winner':'None'}]
									
									with open(os.path.dirname(os.path.realpath(__file__))[:-4]+'gamelists.json','w') as ofile:
										json.dump(gamelists,ofile)
									break	
									return HttpResponse(gamelists)
							################################
			...
			for i in old_accs['accounts']:
				...
				if post_var['username'] == i['username']:
					...
					if passwordh == i['password'] and i['verify']:
						...
						with open(os.path.dirname(os.path.realpath(__file__))[:-4]+'sessions.json','w') as ofile:
