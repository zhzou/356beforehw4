	...
	if request.method == 'GET':
				...
				for i in range(len(gamelist['gamelists'])):
					if username in gamelist['gamelists'][i]:
						...
						if gid == 0 or gamelist['gamelists'][i][username][gid-1]['winner'] != 'None':
							date = datetime.datetime.now().strftime("%y-%m-%d")
							gamelist['gamelists'][i][username] += [{'id':gid+1, 'grid':grid, 'start_date':date,'winner':'None'}]
		...
		if request.POST.get('username'):
			...
			if username != None:
				...
				r = requests.post('http://130.245.169.164/login', data=json.dumps(send_json), headers=headers)
